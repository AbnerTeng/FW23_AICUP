"""
Add social economic village features into training/testing data.
"""
import os
...
def add_social_economic_feature(house: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    village_feature = gpd.read_file(
        f"{os.getcwd()}/data/new_data/social_economic_data/social_economic_feature.shp"
    )
    house_gdf = gpd.GeoDataFrame(
        house,
        geometry = gpd.points_from_xy(house["橫坐標"], house["縱坐標"]), crs = "EPSG:3826"
    )
    house_gdf = gpd.sjoin(house_gdf, village_feature, how = "left")
    return pd.DataFrame(
        house_gdf.drop(columns=["V_ID", "geometry", "index_right"])
    )
